...
function ProductList({ jwt }) {
  ...
  const handleAddToCart = async (product) => {
    try {
      const response = await fetch('http://localhost/ecommerce-app/backend/add_to_cart.php', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        toast.error(`Failed to add ${product.name} to cart: ${data.message}`, { duration: 2000 });
      }
    } catch (error) {
      console.error('Error adding to cart:', error);
      toast.error(`Failed to add ${product.name} to cart: ${error}`, { duration: 2000 });
    }
  };
  ...
  const productsToDisplay = searchTerm ? filteredProducts : (data ? data.products : []);
    ...
    <div className="container mx-auto mt-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {productsToDisplay.map((product) => (
          <div key={product.id} className="relative bg-white shadow-md rounded-md overflow-hidden transform transition-transform hover:scale-105 duration-300">
            <div className="relative">
              <img src={product.image_url} alt={product.name} className="w-full h-64 object-cover rounded-t-md" />
            ...
            <div className="p-4">
              ...
              <button
                className="w-full bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline transition duration-300 flex items-center"
